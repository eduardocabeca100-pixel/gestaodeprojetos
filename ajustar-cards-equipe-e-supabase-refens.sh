...
echo "Criando backup..."
tar -czf ".backup-antes-cards-supabase-refens-$(date +%Y%m%d-%H%M%S).tgz" src supabase .gitignore package.json package-lock.json 2>/dev/null || true

mkdir -p supabase/migrations

cat > supabase/migrations/20260622010000_viva_gestao_refens_core.sql <<'EOF'
...
echo "Atualizando cards de composição dentro da equipe..."
python3 - <<'PY'
...
function getAssignmentCostCards(assignment: LocalProjectAssignment): TeamCostCard[] {
  const key = normalizeCostName(`${assignment.memberId} ${assignment.fullName} ${assignment.name} ${assignment.role} ${assignment.rubric}`);

  if (key.includes("marcel") || key.includes("eduardo cabeca")) {
    return [
      { title: "Diretor geral + produtor", basis: "01 projeto • pré-produção", amount: "R$ 6.000,00" },
      { title: "Professor / formador", basis: "01 projeto • formação teatral", amount: "R$ 2.000,00" },
      { title: "Ator experiente", basis: "03 apresentações x R$ 300,00", amount: "R$ 900,00" },
    ];
  }

  if (key.includes("kaique")) {
    return [
      { title: "Produção executiva", basis: "01 projeto • planejamento, comunicação, execução e acompanhamento", amount: "R$ 6.000,00" },
    ];
  }

  if (key.includes("jones") || key.includes("som")) {
    return [
      { title: "Técnico de som", basis: "03 apresentações x R$ 500,00", amount: "R$ 1.500,00" },
    ];
  }

  if (key.includes("cassius") || key.includes("iluminacao") || key.includes("luz")) {
    return [
      { title: "Técnico de iluminação", basis: "01 serviço técnico", amount: "R$ 500,00" },
    ];
  }

  if (key.includes("andre") || key.includes("fotograf") || key.includes("registro")) {
    return [
      { title: "Registro fotográfico/audiovisual", basis: "01 projeto", amount: "R$ 2.000,00" },
    ];
  }

  if (key.includes("suzi") || key.includes("libras") || key.includes("acessibilidade")) {
    return [
      { title: "Intérprete de LIBRAS", basis: "03 apresentações x R$ 400,00", amount: "R$ 1.200,00" },
      { title: "Capacitação de equipe", basis: "01 capacitação / workshop de inclusão", amount: "R$ 1.000,00" },
    ];
  }

  if (key.includes("katiana") || key.includes("katy") || key.includes("tecladista") || key.includes("vocal")) {
    return [
      { title: "Tecladista / músico", basis: "Apoio musical nas apresentações", amount: "R$ 1.000,00" },
      { title: "Técnica vocal", basis: "Preparação vocal do projeto", amount: "R$ 1.300,00" },
    ];
  }

  if (key.includes("aluno novo") || key.includes("alunos novos") || key.includes("artista em formacao")) {
    return [
      { title: "Aluno novo", basis: "03 apresentações x R$ 50,00", amount: "R$ 150,00" },
    ];
  }

  if (key.includes("ator experiente") || key.includes("atriz experiente") || key.includes("renaldo") || key.includes("bruna") || key.includes("wemerson") || key.includes("julia") || key.includes("karim") || key.includes("karin")) {
    return [
      { title: "Ator/atriz experiente", basis: "03 apresentações x R$ 300,00", amount: "R$ 900,00" },
    ];
  }
    ...
    ? [
        {
          ...
          basis: "Composição não detalhada. Editar cadastro se necessário.",
          amount: assignment.expectedAmount || "R$ 0,00",
        },
      ]
    : [];
}
...
# Adiciona const costCards dentro do map
old = '''                return (
                  <article key={assignment.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">'''
new = '''                const costCards = getAssignmentCostCards(assignment);

                return (
                  <article key={assignment.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">'''
if old in text and "const costCards = getAssignmentCostCards(assignment);" not in text:
    text = text.replace(old, new)

# Insere os cards após os dados de CPF/valores
anchor = '''                        </div>
                      </div>
...
cards_block = '''                        </div>
                        ...
                        {costCards.length > 0 ? (
                          <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                              {costCards.map((card) => (
                                <div key={`${assignment.id}-${card.title}`} className="rounded-2xl border border-slate-200 bg-white p-4">
                                  <p className="text-sm font-black text-slate-950">{card.title}</p>
                                  <p className="mt-1 text-xs leading-5 text-slate-500">{card.basis}</p>
                      ...
                      <div className="flex flex-wrap gap-2">'''
if anchor in text and "Composição do valor previsto" not in text:
    text = text.replace(anchor, cards_block)

path.write_text(text)
PY

echo "Criando helper de conexão/backup para Supabase..."
mkdir -p src/lib/supabase
cat > src/lib/supabase/viva-schema-notes.ts <<'EOF'
...
echo "Conferindo cards e migration..."
grep -R "Composição do valor previsto" -n src/components/team/local-team-workspace.tsx
grep -R "viva_project_team_costs" -n supabase/migrations/20260622010000_viva_gestao_refens_core.sql
grep -R "Diretor geral + produtor" -n supabase/migrations/20260622010000_viva_gestao_refens_core.sql

echo "Tentando enviar migration para o Supabase..."
if npx supabase --version >/dev/null 2>&1; then
  if [ -f "supabase/config.toml" ]; then
    npx supabase db push || echo "AVISO: não consegui aplicar no Supabase automaticamente. Se o projeto não estiver linkado, rode: npx supabase link --project-ref SEU_PROJECT_REF && npx supabase db push"
  else
    echo "AVISO: pasta supabase/config.toml não encontrada. Rode: npx supabase init && npx supabase link --project-ref SEU_PROJECT_REF && npx supabase db push"
  fi
else
...
git add src/components/team/local-team-workspace.tsx src/lib/supabase/viva-schema-notes.ts supabase/migrations/20260622010000_viva_gestao_refens_core.sql .gitignore package.json package-lock.json
git commit -m "Detalha composicao de valores da equipe e estrutura Supabase" || echo "Nada novo para commitar."
